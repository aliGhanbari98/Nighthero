export const authConfig = {}

export const ENVIRONMENT = '' // 'pre-prod-';

export const serverConfig = {
  // baseURL: 'https://' + ENVIRONMENT + 'be.waithero.com',
  // API: 'https://' + ENVIRONMENT + 'be.waithero.com/api',
  baseURL: 'http://' + ENVIRONMENT + '128.140.55.105:3000',
  API: 'http://' + ENVIRONMENT + '128.140.55.105:3000/api',
  requestToken:
    '',
}
